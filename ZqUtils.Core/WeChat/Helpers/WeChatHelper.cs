...
using ZqUtils.Core.WeChat.Models;
/****************************
* [Author] 张强
* [Date] 2015-10-12
* [Describe] 微信工具类
* **************************/
namespace ZqUtils.Core.WeChat.Helpers
{
    /// <summary>
    /// 微信工具类
    /// </summary>
    public class WeChatHelper : IDisposable
    {
            ...
            using (var http = new HttpHelper())
            {
                ...
                if (!string.IsNullOrEmpty(res))
                {
                    ...
                    if (obj != null)
                    {
                        ...
                        req.Url = link;
                        var r = http.GetResult(req).ResultString;
        ...
        public WxUserInfo GetUserInfoByRefreshToken(string refreshToken, Action<OAuthModel> fnOAuthModel = null)
        {
            ...
            using (var http = new HttpHelper())
            {
                ...
                {
                    var obj = result.ToObject<OAuthModel>();
                    if (obj != null)
                    {
                        fnOAuthModel?.Invoke(obj);
                        //根据临时access_token和openid获取用户信息
        ...
        public string AddOtherForeverMaterial(string type, string filePath)
        {
            ...
            if (type != "video")
            {
                ...
                result = http.GetResult().ResultString;
            }
            else
            {
                //新增永久视频素材
